...
# Yuying 20210110
# changed source of NumberOfTrialToReversal from the corresponding raw data col to summing up NumberOfCorrect for each separation width

# Yuying 20201209
# split LD table into probe (TS_LD) and train (TS_LDtrain) tables

# Yuying 20201009
# added empty row evaluation and skipping
...
def EnterRawToDB(InDir,OutFile):
  ...
  for rawsheet in listdir(InDir):
    ...
    for row in c[1:]:
      ...
      # isdigit() ensures that any value other than integers existing in the ITI Touches columns will not be summed
      try:
      ...
      TrialList=[int(row[i].strip('"')) for i in iNumberOfCorrect if str(row[i]).strip('"').isdigit()]
      #print(f'Total {len(TrialList)} trials recorded.')
      ...
      if str(row[iNumberOfTrialToReversal[0]]).strip('"').isnumeric():
        ...
        #if NumberOfTrialTo1stReversal!=len(TrialList):
          #print(f'WARNING: counted "No. Correct" does not match listed "No trials to criterion" in original row {count_original+1}.')
      else:
        if str(row[iNumberOfTrialToReversal[0]]).strip('"')=='':
          #NumberOfTrialTo1stReversal=TrialList.index('')
          NumberOfTrialTo1stReversal=len(TrialList)
        else:
      ...
      if str(row[iNumberOfTrialToReversal[1]]).strip('"').isnumeric():
        ...
        if str(row[iNumberOfTrialToReversal[1]]).strip('"')=='':
          NumberOfTrialTo2ndReversal=len(TrialList)-NumberOfTrialTo1stReversal
        else:
